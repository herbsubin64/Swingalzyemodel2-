...
export default function ComparePage() {
  ...
  const [swingAngle, setSwingAngle] = useState(40); // degrees
        ...
        <label className="flex w-full cursor-pointer flex-col gap-2 rounded border p-3">
          ...
          {leftFile && (
            <span className="text-xs text-gray-600">
              {leftFile.name} ({Math.round((leftFile.size / 1024 / 1024) * 10) / 10} MB)
            </span>
          )}
        </label>
        <label className="flex w-full cursor-pointer flex-col gap-2 rounded border p-3">
          ...
          {rightFile && (
            <span className="text-xs text-gray-600">
              {rightFile.name} ({Math.round((rightFile.size / 1024 / 1024) * 10) / 10} MB)
            </span>
          )}
        </label>
      </div>
